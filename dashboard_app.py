...
def load_all_data():
    ...
    for p in [DATA_DIR / "clean_jobs.parquet", DATA_DIR / "dashboard_ready_data.parquet", DATA_DIR / "clean_jobs.csv"]:
        if p.suffix == ".parquet":
            jobs = safe_read_parquet(p)
        else:
            jobs = safe_read_csv(p)
        if jobs is not None:
            break
    ...
    student_parquet = safe_read_parquet(DATA_DIR / "clean_students.parquet")
    student_csv = safe_read_csv(DATA_DIR / "clean_students.csv")
    student_xlsx = None
    if (DATA_DIR / "student_data.xlsx").exists():
        student_xlsx = pd.read_excel(DATA_DIR / "student_data.xlsx")

    students = student_parquet if student_parquet is not None else student_csv
    if students is None:
        students = student_xlsx
    if students is None:
